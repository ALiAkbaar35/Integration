...
export const ContractProvider = ({ children }) => {
    ...
    for (let i = 0; i < totalItems; i++) {
      ...
      try {
        ...
        if (error.code === 409) {
          ...
          fileContent += `${docId}\t${departmentItemData[i].code}\t\t${departmentItemData[i].description}\t\t\t${error.message}\n`;
  ...
  const vendors = async (vendor) => {
    let userId = getUserId();
    const totalItems = vendor.length;
    let fileContent = "ID\tCode\t\tName\t\t\t\terror\n\n"; 
    ...
    for (let i = 0; i < totalItems; i++) {
      ...
      try {
        ...
        if (error.code === 409) {
          ...
          fileContent += `${vendor[i].id}\t${vendor[i].code}\t\t${vendor[i].name}\t\t\t${error.message}\n`;
        } else {
  ...
  const properties = async (propertiesItemData) => {
    let userId = getUserId();
    const totalItems = propertiesItemData.length;
    let fileContent = "ID\tCode\t\tName\t\t\t\terror\n\n"; 
    ...
    for (let i = 0; i < totalItems; i++) {
      ...
      try {
        ...
        if (error.code === 409) {
          ...
          fileContent += `${docId}\t${propertiesItemData[i].code}\t\t${propertiesItemData[i].description}\t\t\t${error.message}\n`;
        } else {
  ...
  const rent_contracts = async (contract) => {
    let userId = getUserId();
    const totalItems = contract.length;
    let fileContent = "ID\tmonthlyInstallment\t\t\tpaymentStartDate\t\t\t\terror\n\n"; 
    for (let i = 0; i < totalItems; i++) {
      ...
      vendorString = vendorIds.toString();
        try {
          await database.createDocument(Database, Contract_db, propString, {
            users: userId,
          ...
          if (error.code === 409) {
            message.error("Properties name or code already exists");
            fileContent += `${docId}\t${contract[i].monthly_installment}\t\t\t\t\t${contract[i].payment_start_date}\t\t\t${error.message}\n`;
          } else {
            message.error("contract " + contract[i].id + " Failed to add");
            console.error("Error creating properties item:", error);
            fileContent += `${docId}\t${contract[i].monthly_installment}\t\t\t\t\t${contract[i].payment_start_date}\t\t\t${error.message}\n`;
          }
        }
   ...
   const schedule= async (schedule) => {
    ...
    let fileContent = "ID\tCode\t\tpayment_amount\t\t\t\terror\n\n"; 
     ...
     for (let i = 0; i < totalItems; i++) {
      ...
      try {
        ...
        if (error.code === 409) {
          ...
          fileContent += `${docId}\t${schedule[i].agreement}\t\t${schedule[i].payment_amount}\t\t\t${error.message}\n`;
        } else {
